...
local Window = Library.CreateLib("Ymir.cc [BETA] | "..game.PlaceId, "DarkTheme")
...
Section:NewToggle("DEFAULT ESP", ".", function(state)
    if state then
...
Section:NewButton("COLORS", ".", function()
    
    
    
    
    Section:NewToggle("GREEN", ".", function(state)
    if state then
...
for i,v in pairs(players) do
 ...
 Section:NewToggle("YELLOW", ".", function(state)
    if state then
...
for i,v in pairs(players) do
	...
	esp.Name = v.Name
	esp.FillTransparency = 0.4
	esp.FillColor = Color3.new(0.94902, 1, 0)
	esp.OutlineColor = Color3.new(0, 0, 0)
	esp.OutlineTransparency = 0
	esp.Parent = v.Character
end
